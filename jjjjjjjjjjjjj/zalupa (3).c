...
static const struct option long_options_list[] = {
    ...
    {"shift", no_argument, NULL, 1488},
    {"step", required_argument, NULL, 1489},
    {"axis", required_argument, NULL, 1490},
    {0, 0, 0, 0}
};
...
int parse_args(int argc, char* argv[], Options* opts){
    ...
    while((current_opt = getopt_long(argc, argv, "hi:o:", long_options_list, NULL)) != -1){
        ...
        switch (current_opt){
            ...
            case 1488: opts->mode = 1488; break;
            ...
            case 1489: if(sscanf(optarg, "%d", &(opts->step)) != 1) return 0; break;
            case 1490: if(strcmp(optarg, "x") != 0 && strcmp(optarg, "y") != 0 && strcmp(optarg, "xy") != 0) return 0; strcpy(opts->axis, optarg); break;
        ...
        if (prev_mode != MODE_UNKNOWN && opts->mode != prev_mode &&
            (opts->mode == MODE_RECTANGLE || opts->mode == MODE_HEXAGON || opts->mode == MODE_COPY || opts->mode == MODE_INFO || opts->mode == MODE_EXPAND) ) {
...
void nakakal(BMPFile* bmpf, char* axis, int step){
    int width = bmpf->bih.width;
    int height = bmpf->bih.height;


    RGB** new_image = malloc(sizeof(RGB*) * bmpf->bih.height);

    for (unsigned int y = 0; y < bmpf->bih.height; y++) {
        new_image[y] = malloc(sizeof(RGB) * bmpf->bih.width);
    }
    ...
    for (int y = 0; y < height; y++){
        for (int x = 0; x < width; x++){
            int nx = x, ny = y;
            if(strchr(axis, 'x')) {
                nx = (x + step % width+width)%width;
            }
            if(strchr(axis, 'y')) {
                ny = (y + step % height+height)%height;
            }

            new_image[ny][nx].r = bmpf->rgb[y][x].r;
            new_image[ny][nx].g = bmpf->rgb[y][x].g;
            new_image[ny][nx].b = bmpf->rgb[y][x].b;
        }
    }
    bmpf->rgb = new_image;
}
...
int main(int argc, char* argv[]){
    ...
    switch (options_config.mode){
        ...
        case MODE_EXPAND:
            ...
            break;
        case 1488:
            nakakal(bmp_image_data, options_config.axis, options_config.step);
            writeBMPFile(options_config.output_name, bmp_image_data);
            break;
        default:
    ...
    if (options_config.mode != MODE_INFO) {
